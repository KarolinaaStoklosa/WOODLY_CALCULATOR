    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 p-4 md:p-6">
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-xl p-3 border border-white/20 shadow-md mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={handleAddDrzwi} disabled={!isEditMode} className="group relative overflow-hidden bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-5 py-2.5 rounded-lg font-semibold shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 disabled:bg-gray-400 disabled:cursor-not-allowed"><div className="relative flex items-center gap-2 text-sm"><Plus className="w-4 h-4" /><span>Dodaj drzwi</span></div></button>
      ...
      {drzwiPrzesuwne.length === 0 ? (
        ...
        <div className="space-y-4">
          {drzwiPrzesuwne.map((drzwi, index) => <DrzwiPrzesuwneCard key={drzwi.id} drzwi={drzwi} index={index} onUpdate={handleUpdateDrzwi} onRemove={handleRemoveDrzwi} showAdvanced={showAdvanced} drzwiOptions={drzwiOptions} formatPrice={formatPrice} isEditMode={isEditMode} />)}
        </div>
      )}
      {drzwiPrzesuwne.length  > 0 &&
        <div className="pt-2">
          <button onClick={handleAddDrzwi} disabled={!isEditMode} className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-blue-500 hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
             <Plus size={16} />
              <span className="text-sm font-semibold">Dodaj nowe drzwi przesuwne poniżej</span>
          </button>
        </div> }
    </div>
  );
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                        ...
                        <select value={drzwi.rodzaj} onChange={(e) => onUpdate(drzwi.id, 'rodzaj', e.target.value)} disabled={!isEditMode} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all text-sm disabled:bg-gray-100 disabled:cursor-not-allowed">{drzwiOptions.map((option, idx) => (<option key={idx} value={option.nazwa}>{option.nazwa}</option>))}</select>
                    </div>
                    <div className="w-24">
                        <label className="block text-xs font-medium text-gray-600 mb-1">Ilość</label>
                        <input type="number" value={drzwi.ilość} onChange={(e) => onUpdate(drzwi.id, 'ilość', e.target.value)} disabled={!isEditMode} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all text-center text-sm disabled:bg-gray-100 disabled:cursor-not-allowed" placeholder="1" min="1" />
                    </div>
                </div>
                <div className="flex items-center gap-2 ml-4">
                    ...
                    <button onClick={() => onRemove(drzwi.id)} disabled={!isEditMode} className="w-9 h-9 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg flex items-center justify-center transition-colors flex-shrink-0 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed"><Trash2 className="w-4 h-4" /></button>
                </div>
            </div>
        </div>
    </div>
